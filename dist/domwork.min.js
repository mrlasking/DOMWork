...
return __module0__}();return dw.Handlebars=dw.Handlebars||Handlebars,"Use it from $DW directly: $DW.Handlebars"},needToRun:!0})}($DW),!function(dw){dw().plugin({name:"pubsub",func:function(){var ps=function(){var subToken=0,topics={};this.publish=function(topic,obj){return topics[topic]?void topics[topic].forEach(function(item){item.func(obj)}):!1},this.subscribe=function(topic,callback){topics[topic]||(topics[topic]=[]);var token=++subToken;return topics[topic].push({token:token,func:callback}),token},this.unsubscribe=function(token){for(var m in topics)if(topics[m])for(var i=0,l=topics[m].length;l>i;i++)topics[m][i].token===token&&topics[m].splice(i,1)}};return dw.pubsub=dw.pubsub||new ps,"Use it from $DW directly: $DW.pubsub.publish, $DW.pubsub.subscribe, $DW.pubsub.unsubscribe"},needToRun:!0})}($DW),!function(dw){dw().plugin({name:"hide",func:function(){return this.elements.forEach(function(elem){elem.style.display="none"}),this}}),dw().plugin({name:"show",func:function(){return this.elements.forEach(function(elem){elem.style.display=""}),this}})}(window.$DW);